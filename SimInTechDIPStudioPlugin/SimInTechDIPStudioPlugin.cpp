...
/* Заглушка для одной posix-функции */
...
/* Заглушка для недостающей в MSVC функции */
void sincos(g_real_type x, g_real_type *sinx, g_real_type *cosx) {
	*sinx = sin(x);
	*cosx = cos(x);
};
...
)
{
	int ret;
	ret = 0;
	Initializer::Init(step, modeltime, ext_vars_addr, din_vars, derivates, alg_vars, alg_funcs, state_vars, consts, locals, solver_data, algo_object_id);
	return ret;
};
...
{
	int ret;
	NativeWrapper::CallPlugin(action, step, modeltime, ext_vars_addr, din_vars, derivates, alg_vars, alg_funcs, state_vars, consts, locals, solver_data, algo_object_id);
	ret = 0;
	return ret;
};
...
)
{
	...
	ret = 0;
	State::Run(action, step, modeltime, ext_vars_addr, din_vars, derivates, alg_vars, alg_funcs, state_vars, consts, locals, solver_data, algo_object_id);
	return ret;
};
